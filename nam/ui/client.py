...
class NAMRPCProtocol(Protocol):
    ...
    def dataReceived(self, data):
        ...
        while data:
            ...
            if message_type == RPC_RESPONSE:
                ...
                # Create the NAMRPCError to pass to the errback
                r = self.__rpc_requests[request_id]
                e = NAMRPCError(r.method, r.args, r.kwargs, request[2][0], request[2][1], request[2][2])
    ...
    def send_request(self, request):
        ...
        """
        # Store the NAMRPCRequest object just in case a RPCError is sent in
...
class NAMRPCClientFactory(ClientFactory):
    protocol = NAMRPCProtocol
...
class DaemonSSLProxy(DaemonProxy):
    def __init__(self, event_handlers={}):
        self.__factory = NAMRPCClientFactory(self, event_handlers)
    ...
    def __rpcError(self, error_data):
        ...
        :param error_data: this is passed from the deferred errback with error.value
            containing a `:class:NAMRPCError` object.
        """
        # Get the NAMRPCError object from the error_data
...
class Client(object):
    ...
    def connect(self, host="127.0.0.1", port=56746, username="", password=""):
        ...
        if not username and host in ("127.0.0.1", "localhost"):
            ...
            username = "localuser"
            password = open(
                nam.configmanager.get_config_dir("auth"), "r"
            ).readline().strip()
    ...
    def start_daemon(self, port, config):
        ...
        try:
            if nam.common.windows_check():
                win32api.WinExec(
                    "nam-daemon --port=%s --config=%s" % (port, config)
                )
            elif nam.common.osx_check():
                subprocess.call([
                    "nohup", "nam-daemon", "--port=%s" % port,
                    "--config=%s" % config
                ])
            else:
                subprocess.call([
                    "nam-daemon", "--port=%s" % port, "--config=%s" % config
                ])
        except OSError, e:
            log.exception(e)
            raise e
