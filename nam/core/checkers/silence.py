...
from nam.database import MessageKind, Message
...
class SilenceChecker(component.Component):
    ...
    def __unicode__(self):
        return u'<SilenceChecker Source "%s">' % getattr(
            self.source, 'src_name', self.source_id
        )
    ...
    def prepare(self):
        ...
        self.check_bus_level_messages_id = self.bus.connect(
            'message::element', self.check_bus_level_messages
        )
    ...
    def prepare_reverse(self):
        ...
        self.audio_failure_both_persists_right_ok = False
        self.audio_failure_both_persists_left_ok = False
        self.bus.disconnect(self.check_bus_level_messages_id)

        self.tee.unlink(self.queue)
        self.queue.unlink(self.level)
        self.level.unlink(self.sink)
        self.pipeline.remove_many(self.queue, self.level, self.sink)
        self.gst_setup_complete = False
...
#                log.trace("L and R > Silence")
    ...
    def update_details(self, silence_checker):
        if silence_checker.min_tolerance != self.min_tolerance:
            log.debug("Source %s silence min tolerace changed from %s to %s",
                      self.source.src_name, self.min_tolerance,
                      silence_checker.min_tolerance)
            self.min_tolerance = silence_checker.min_tolerance


        if silence_checker.max_tolerance != self.max_tolerance:
            log.debug("Source %s silence max tolerace changed from %s to %s",
                      self.source.src_name, self.max_tolerance,
                      silence_checker.max_tolerance)
            self.max_tolerance = silence_checker.max_tolerance


        if silence_checker.silence_level != self.silence_level:
            log.debug("Source %s silence silence level changed from %s to %s",
                      self.source.src_name, self.silence_level,
                      silence_checker.silence_level)
            self.silence_level = silence_checker.silence_level
