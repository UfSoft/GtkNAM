...
from nam import error, rencode

import nam.component as component
import nam.configmanager
from nam.database import AUTH_LEVEL_NONE, AUTH_LEVEL_DEFAULT
...
class ServerContextFactory(object):
    def getContext(self):
        ...
        SSL transport.
        """
        ssl_dir = nam.configmanager.get_config_dir("ssl")
...
class NAMRPCProtocol(Protocol):
    ...
    def connectionMade(self):
        ...
        log.info("NAM Client connection made from: %s:%s", peer.host, peer.port)
    ...
    def connectionLost(self, reason):
        ...
        log.info("NAM client disconnected: %s", reason.value)
    ...
    def dispatch(self, request_id, method, args, kwargs):
        ...
        if method in self.factory.methods and \
            ...
            try:
                ...
                sendError()
                # Don't bother printing out NAMErrors, because they are just for the client
                if not isinstance(e, error.NAMError):
...
class RPCServer(component.Component):
    ...
    def __init__(self, port=56746, interface="", allow_remote=False, listen=True):
        component.Component.__init__(self, "RPCServer")

        self.factory = Factory()
        self.factory.protocol = NAMRPCProtocol
        ...
        log.info("Starting NAMRPC server %s:%s", hostname, port)
