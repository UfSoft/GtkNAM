...
def get_version():
    ...
    """
    import pkg_resources
    return pkg_resources.require("NetworkAudioMonitor")[0].version
...
def get_default_config_dir(filename=None):
    ...
    if windows_check():
        if filename:
            return os.path.join(os.environ.get("APPDATA"), "nam", filename)
        else:
            return os.path.join(os.environ.get("APPDATA"), "nam")
    else:
        import xdg, xdg.BaseDirectory
        if filename:
            return os.path.join(xdg.BaseDirectory.save_config_path("nam"), filename)
        else:
            return xdg.BaseDirectory.save_config_path("nam")
...
def get_pixmap(fname):
    ...
    return pkg_resources.resource_filename(
        "nam", os.path.join("data", "pixmaps", fname)
    )
...
def ftimenano(nanoseconds):
    return ftime(nanoseconds/1000000000)
...
def get_localhost_auth():
    ...
    :rtype: tuple
    """
    auth_file = nam.configmanager.get_config_dir("auth")
