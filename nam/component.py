...
class Component(object):
    ...
    **States:**
    ...
    def __init__(self, name, interval=1, depend=None):
        ...
        self._component_state = STATE_STOPPED
    ...
    def _component_start(self):
        def on_start(result):
            self._component_state = STATE_STARTED
            self._component_starting_deferred = None
        ...
        if self._component_state == STATE_STOPPED:
            if hasattr(self, "start"):
                self._component_state = STATE_STARTING
        ...
        elif self._component_state == STATE_STARTING:
            return self._component_starting_deferred
        elif self._component_state == STATE_STARTED:
            d = succeed(True)
        else:
    ...
    def _component_stop(self):
        ...
        if self._component_state != STATE_STOPPED and \
                                        self._component_state != STATE_STOPPING:
        ...
        if self._component_state == STATE_STOPPING:
    ...
    def _component_pause(self):
        def on_pause(result):
            self._component_state = STATE_PAUSED

        if self._component_state == STATE_STARTED:
            if self._component_timer and self._component_timer.running:
                d = maybeDeferred(self._component_timer.stop)
                d.addCallback(on_pause)
            else:
                d = succeed(None)
        elif self._component_state == STATE_PAUSED:
            d = succeed(None)
        else:
    ...
    def _component_resume(self):
        ...
        if self._component_state == STATE_PAUSED:
...
class ComponentRegistry(object):
    ...
    def pause(self, names=[]):
        ...
        *names* is specified, then it will only pause those Components,
        ...
        for name in names:
            if self.components[name]._component_state == STATE_STARTED:
    ...
    def resume(self, names=[]):
        ...
        *names* is specified, then it will only resume those Components,
        ...
        for name in names:
            if self.components[name]._component_state == STATE_PAUSED:
